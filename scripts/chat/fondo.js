...
(function () {
  ...
  // Ignorar nuestras propias escrituras evita un ciclo MutationObserver/rAF.
  function cambioExterno(records) {
    return records.some(record => {
      if (record.target.closest?.('#p5-svg-conexiones')) return false;
      return [...record.addedNodes, ...record.removedNodes].some(node =>
        node.nodeType !== 1 || node.id !== 'p5-svg-conexiones'
      );
    });
  }
  ...
  function actualizar() {
    if (actualizacionPendiente || document.hidden) return;
  ...
  function observarPanel(panel) {
    ...
    observerPanel = new MutationObserver(records => {
      if (cambioExterno(records)) actualizar();
    });
  ...
  function buscarPanel() {
    ...
    observerPanel?.disconnect();
    document.getElementById('p5-svg-conexiones')?.remove();

    panelActual = panel;
    if (panel) observarPanel(panel);
  }
  ...
  function iniciar() {
    ...
    if (!observerGeneral) {
      observerGeneral = new MutationObserver(records => {
        if (cambioExterno(records)) buscarPanel();
      });
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden && frameId !== null) {
        ...
        frameId = null;
        actualizacionPendiente = false;
      } else if (!document.hidden) actualizar();
    });
